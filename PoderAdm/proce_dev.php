...
if (($resultado_prod) AND ($resultado_prod->num_rows != 0)) {
    while ($row_prod = mysqli_fetch_assoc($resultado_prod)) {
        ...
        echo "<li>" . $row_prod['nome'] . "<button name='$row_prod[quant_estoque]' id='$row_prod[nome]' class='$row_prod[numero_caixa]' onclick='devolver(id, name, $row_prod[numero_caixa])'>+</button></li>";
    }
} else {
...
$script = "
    <script>
       function devolver(id, name, numero_caixa){
            var devolve    = document.getElementById('devolve');
            ...
            var tdevolv     = document.createElement('td');
